...
class TestRedForNDays(unittest.TestCase):
    ...
    def test_build_red_for_n_days_filters_all_red_by_default(self) -> None:
        ...
        self.assertEqual(result["min_red"], 3)
        self.assertEqual(result["candidate_count"], 3)
        self.assertEqual(result["matched_count"], 1)
        self.assertEqual(result["insufficient_kline_count"], 1)
        self.assertEqual(result["matches"][0]["code"], "000001")
        self.assertEqual(result["matches"][0]["red_count"], 3)
        self.assertEqual(result["matches"][0]["gain_n_days_pct"], 6.0)
        self.assertEqual(result["matches"][0]["bars"][1]["close"], 10.2)

    def test_build_red_for_n_days_min_red_allows_partial_red(self) -> None:
        """min_red=2 out of 3 should include 000002 (2 red candles) but still exclude 000003."""
        from app.pipelines.red_for_n_days import build_red_for_n_days
        ...
        self.assertEqual(result["min_red"], 2)
        codes = {m["code"] for m in result["matches"]}
        self.assertIn("000001", codes)
        self.assertIn("000002", codes)
        match_002 = next(m for m in result["matches"] if m["code"] == "000002")
        self.assertEqual(match_002["red_count"], 2)
